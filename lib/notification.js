...
// TODO: Each query should probably only return the notifications meant for a user.
...
Notification.prototype.update = function(cb) {
  ...
  pool.getConnection(function(err, connection) {
    ...
    // Use the connection
    var sql = 'UPDATE ' + TABLE_NAME + ' SET ' + connection.escape(self.toObject()) + ' WHERE ID = ' + connection.escape(self.toObject().ID);
    if (debug) console.log("DEBUG-SQL: " + sql);
...
Notification.prototype.index = function(opts, cb) {
  ...
  pool.getConnection(function(err, connection) {
    ...
    // Use the connection
    var sql = 'SELECT * FROM ' + TABLE_NAME + ' WHERE ToUserID = ' + connection.escape(self.toObject().ToUserID) + ' ORDER BY TimeCreated DESC LIMIT ' + connection.escape(opts.start) + ',' + connection.escape(opts.limit) ;
    if (debug) console.log("DEBUG-SQL: " + sql);
