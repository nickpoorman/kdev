...
Notification.prototype.index = function(opts, cb) {
  ...
  pool.getConnection(function(err, connection) {
    ...
    // Use the connection
    var sql = 'SELECT * FROM ' + TABLE_NAME + ' WHERE ToUserID = ' + connection.escape(self.toObject().ToUserID) + ' LIMIT ' + connection.escape(opts.start) + ',' + connection.escape(opts.limit) + ' ORDER BY TimeCreated DESC';
    if (debug) console.log("DEBUG-SQL: " + sql);
